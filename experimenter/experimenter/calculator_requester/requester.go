...
func (cr *requester) RequestAdd(data map[string]any) (map[string]any, error, time.Duration) {
	return http_utils.PostRequest(fmt.Sprintf("%s/add", cr.url), data)
}

// RequestSubtract requests the subtract method from the API.
func (cr *requester) RequestSubtract(data map[string]any) (map[string]any, error, time.Duration) {
...
func (cr *requester) RequestMultiply(data map[string]any) (map[string]any, error, time.Duration) {
	return http_utils.PostRequest(fmt.Sprintf("%s/multiply", cr.url), data)
}

// RequestDivide requests the divide method from the API.
func (cr *requester) RequestDivide(data map[string]any) (map[string]any, error, time.Duration) {
